...
import { googleApiErrorStatus, withGoogleApiRetry } from "../../src/core/google-api-retry.js";
...
describe("googleApiErrorStatus", () => {
  it("reads the numeric HTTP status gaxios sets on .status, not .code", () => {
    ...
    expect(googleApiErrorStatus(gaxiosLikeError(404))).toBe(404);
    expect(googleApiErrorStatus(gaxiosLikeError(429))).toBe(429);
  });

  it("is undefined for a plain Error with no status", () => {
    expect(googleApiErrorStatus(new Error("boom"))).toBeUndefined();
  });
});
...
describe("withGoogleApiRetry", () => {
  ...
  it("gives up after maxAttempts and throws the last error", async () => {
    ...
    await expect(
      withGoogleApiRetry(fn, { maxAttempts: 3, baseDelayMs: 1, maxDelayMs: 2 })
    ).rejects.toBeDefined();
    expect(fn).toHaveBeenCalledTimes(3);
  });
  ...
  it("honors a Retry-After header instead of the computed backoff", async () => {
    ...
    const result = await withGoogleApiRetry(fn, { baseDelayMs: 10_000, maxDelayMs: 20_000 });
