 ...
 * payload.partId/filename and a message-level sizeEstimate) must normalize
 * identically to one pre-trimmed to exactly the fields that selector
 * requests, down through a 3-level nested MIME tree well within
...
describe("gmail/scanner.ts", () => {
  ...
  it("preserves all 16 scenarios", async () => {
    await runScenarios([
      ...
      { name: "historyIdGreaterThan handles values beyond Number.MAX_SAFE_INTEGER", run: () => {
        expect(historyIdGreaterThan("9007199254740993", "9007199254740992")).toBe(true);
      } },
      { name: "historyIdGreaterThan is false for equal values", run: () => {
        expect(historyIdGreaterThan("42", "42")).toBe(false);
      } },
      { name: "listAllMessageIds trims the result to exactly the cap even when a single page over-fills it", run: async () => {
        ...
        const result = await listAllMessageIds(client, { labelIds: ["INBOX"], includeSpamTrash: false, safetyCapCount: 2 });
        expect(result.messages).toHaveLength(2);
        expect(result.messages.map((m) => m.id)).toEqual(["a", "b"]);
        expect(result.truncated).toBe(true);
        expect(result.estimatedTotal).toBe(500);
      } },
      { name: "listAllMessageIds does not paginate further once the cap is reached", run: async () => {
        const client = fakeListClient([
      ...
      { name: "listAllMessageIds is not truncated when everything fits under the cap", run: async () => {
        const client = fakeListClient([{ messages: [stub("a")], resultSizeEstimate: 1 }]);
        const result = await listAllMessageIds(client, { labelIds: ["INBOX"], includeSpamTrash: false, safetyCapCount: 10 });
        expect(result.truncated).toBe(false);
        expect(result.messages).toHaveLength(1);
      } },
      { name: "listAllMessageIds without a cap fetches everything across pages", run: async () => {
        const client = fakeListClient([
      ...
      { name: "listHistorySince preserves each changed message's threadId from the history record, not just its id", run: async () => {
        const client = fakeHistoryClient([
          { history: [{ id: "150", labelsAdded: [{ message: { id: "m1", threadId: "t1" } }] }], historyId: "150" }
        ]);
      ...
      { name: "listHistorySince removes a message from changedMessages once it appears in messagesDeleted", run: async () => {
        const client = fakeHistoryClient([
          {
            history: [
      ...
      { name: "listHistorySince reports an expired marker on a 404 rather than throwing", run: async () => {
        const client = {
          users: { history: { list: async () => { throw Object.assign(new Error("not found"), { status: 404 }); } } }
        } as unknown as GmailClient;
      ...
      { name: "fetchInboxMessageCount returns messagesTotal from a single users.labels.get call", run: async () => {
        const client = { users: { labels: { get: async () => ({ data: { messagesTotal: 37 } }) } } } as unknown as GmailClient;
        expect(await fetchInboxMessageCount(client)).toBe(37);
      } },
      { name: "fetchInboxMessageCount defaults to 0 when messagesTotal is absent", run: async () => {
        const client = { users: { labels: { get: async () => ({ data: {} }) } } } as unknown as GmailClient;
        expect(await fetchInboxMessageCount(client)).toBe(0);
      } },
      { name: "fetchMessageFull requests format=full narrowed by MESSAGE_FULL_FIELDS", run: async () => {
        let capturedParams: unknown;
        const client = {
          users: { messages: { get: async (params: unknown) => { capturedParams = params; return { data: { id: "m1", threadId: "t1" } }; } } }
        } as unknown as GmailClient;
        await fetchMessageFull(client, "m1");
        expect(capturedParams).toMatchObject({ format: "full", fields: expect.stringContaining("payload(") });
      } },
      { name: "listAllMessageIds and fetchProfile request their documented field selectors", run: async () => {
        let listParams: unknown;
        let profileParams: unknown;
        const client = {
          users: {
            messages: { list: async (params: unknown) => { listParams = params; return { data: { messages: [], resultSizeEstimate: 0 } }; } },
            getProfile: async (params: unknown) => { profileParams = params; return { data: { emailAddress: "me@example.com", historyId: "1" } }; }
          }
        } as unknown as GmailClient;
      ...
      { name: "a response narrowed to exactly MESSAGE_FULL_FIELDS normalizes identically to the untrimmed original", run: () => {
            ...
            parts: [
              {
                ...
                body: { size: 0 },
                parts: [
                  { partId: "0.0.0", mimeType: "text/plain", filename: "", body: { size: 5, data: Buffer.from("plain").toString("base64url") }, parts: [] },
                  { partId: "0.0.1", mimeType: "text/html", body: { size: 15, data: Buffer.from("<p>html</p>").toString("base64url") }, parts: [] }
                ]
              }
            ]
          }
      ...
      { name: "listSentThreadIds builds a local reply-protection index from cheap message stubs", run: async () => {
        const client = fakeListClient([
          { messages: [stub("sent-1"), { id: "sent-2", threadId: "shared-thread" }], resultSizeEstimate: 2 }
        ]);
