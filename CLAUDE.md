...
- Pace Gmail/Calendar attempts through one adaptive process-wide limiter, starting at 3 baseline-equivalent messages/second and recovering toward 4 after sustained success unless `GMAIL_AGENT_RATE_LIMIT_RPS` explicitly fixes the rate. Calls reserve quota-weighted slots (`messages.get` 20 units, `threads.get` 40, `messages.list` 5, history 2, labels 1, and expensive sends/batches weighted accordingly). Quota-shaped failures halve throughput immediately; ordinary network errors reset the recovery streak without being mislabeled as quota pressure.
