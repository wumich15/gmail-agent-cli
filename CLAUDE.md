...
- cache by normalized-content hash plus model, schema, prompt, and policy versions;
...
- `messages`: Gmail IDs, thread ID, immutable metadata/content hash, label snapshot, classifier/prompt/schema versions, minimal assessment projection, processed timestamp.
...
- Use bounded concurrency, initially 5 Gmail reads, 2 AI calls, and 2 Calendar writes; make limits configurable.
- Cache assessments using content and version hashes. Explicit spam/native-spam decisions avoid AI entirely; important and read-archive decisions still invoke event extraction when deterministic cues indicate it may be needed.
- Retry Google 429 and transient 5xx responses with truncated exponential backoff and jitter. Honor `Retry-After`.
