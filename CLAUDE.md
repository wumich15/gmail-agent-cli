...
The cache command renders a progress bar for discovery, its 8-worker quota-paced hydration pass, and final marker reconciliation on stderr, so the entire cache operation visibly advances rather than appearing blank until completion.
...
- Use bounded concurrency, currently 5 Gmail reads and 5 AI calls by default for `gmail work`; the explicit full-snapshot `gmail cache` hydration pool uses 8 Gmail reads so network latency is overlapped more aggressively while the shared weighted limiter still enforces the quota budget. `mapWithConcurrency` is a true worker pool: a completed item immediately frees its slot instead of a slow request holding an entire fixed batch behind it. Gmail and AI retain separate concurrency budgets.
...
- Pace Gmail/Calendar attempts through one adaptive process-wide limiter, starting at 5 baseline-equivalent messages/second and recovering toward 10 (200 `messages.get`-equivalent quota units/second) after sustained success unless `GMAIL_AGENT_RATE_LIMIT_RPS` explicitly fixes the rate. This reaches the headroom available to legacy Gmail projects while newer 6,000-units/minute projects automatically halve throughput on their first quota-shaped response. Calls reserve quota-weighted slots (`messages.get` 20 units, `threads.get` 40, `messages.list` 5, history 2, labels 1, and expensive sends/batches weighted accordingly). Ordinary network errors reset the recovery streak without being mislabeled as quota pressure.
...
- Group label mutations, but do not assume batches save quota units or execute in order. Gmail's HTTP batch facility can reduce connection overhead, but Google still counts each inner call toward quota and recommends keeping batches at 50 calls or fewer; the current code therefore relies on `batchModify` for mutations and on list/history synchronization for reads.
...
### Planned Gmail read-transport optimization (implemented, opt-in pending live-account benchmark)

**Implementation status:** steps 1-5 below are implemented — partial-response `fields` selectors on every read call (`gmail/scanner.ts`), the multipart batch transport (`gmail/batch.ts`) with deterministic tests for mixed status codes, out-of-order parts, missing `Content-ID`, and malformed boundaries, the quota-aware batching driver (`gmail/batch-hydrate.ts`, reserving quota per inner call, retrying only retryable failed parts, falling back to individual reads on a structural batch failure), and a real local-server test proving gzip request headers/decompression work through the custom transport (`tests/unit/gzip-batch.test.ts`). `gmail cache` uses this path only when `GMAIL_AGENT_BATCH_HYDRATION=1` is set; step 6's default-on rollout has not happened because the acceptance gate below requires a live-account 200/500-message benchmark this development environment cannot run — do not flip the default without that evidence. Byte-level response-size and limiter-wait-vs-network-time instrumentation from step 1 is not yet implemented; the current diagnostics report outer batch request counts and individual-fallback counts only.

This plan is based on Google's [Gmail batch-request guide](https://developers.google.com/workspace/gmail/api/guides/batch) and [Gmail performance guide](https://developers.google.com/workspace/gmail/api/guides/performance). Batching and gzip are transport optimizations, not quota bypasses: a batch containing `n` inner calls still counts as `n` calls, batches may execute their parts in any order, and Google warns that batches larger than 50 are more likely to trigger rate limiting. The implementation must therefore reduce HTTP setup and response bytes while continuing to budget every `messages.get` at its full quota cost.

The installed Node `googleapis-common` client already adds `Accept-Encoding: gzip` and a user agent containing `(gzip)` for ordinary non-browser requests. Do not add a second competing compression layer to the current client. First verify this behavior with an integration test and content-free transport metrics; any custom batch transport introduced below must explicitly preserve those two headers and prove that gzip responses are decompressed correctly.

Implementation sequence:

1. **Measure the current path.** Add content-free counters for discovered IDs, successful and failed hydrations, retry/quota failures, outer HTTP requests, response bytes when available, and time spent waiting for the quota limiter versus Gmail network time. The cache progress display must distinguish `Gmail quota cooldown` from a hung request. Benchmark uncached runs of at least 200 and 500 messages before changing transport.
2. **Apply partial responses before batching.** Add Gmail `fields` selectors to list/profile/history/label calls and to `messages.get`. Preserve every field normalization and safety policy actually consumes: message/thread/history IDs, internal date, label IDs, snippet, and the MIME payload/header/body/part tree. Exclude unused response fields and never fetch attachment bytes during a normal cache/work run. Add fixtures proving the narrowed response produces identical normalized messages, content hashes, classifier inputs, and policy decisions.
3. **Introduce a dedicated Gmail multipart batch reader.** Add a transport module that obtains authorization from the existing OAuth client, resolves Gmail v1's batch path, emits `multipart/mixed` requests containing only relative `users/me/messages/<id>?format=full&fields=...` paths, and maps responses by `Content-ID` rather than assuming execution order. Start at 25 inner `messages.get` calls per outer request, make the size configurable, and enforce a hard maximum of 50 even though Gmail's protocol maximum is 100.
4. **Keep batching quota-aware.** Before sending an outer batch, reserve quota for every inner call in the shared limiter; an outer batch must never be accounted as one cheap request. Send at most one hydration batch at a time initially. On a 429 or per-minute quota error, enter one shared cooldown, reduce subsequent batches to 10 and then 5 parts, honor bounded `Retry-After`, and retry only retryable failed parts—not successful parts or the entire batch. A malformed multipart response, missing part, unsupported endpoint, or authentication error must fall back safely to bounded individual reads without losing already-successful results.
5. **Preserve cache correctness and progress.** Feed each decoded part through the existing normalization/upsert path, advance progress per completed message, deduplicate by Gmail message ID, and retain the rule that a history baseline advances only after an untruncated snapshot with zero missing/failed messages. Never persist bodies, multipart payloads, authorization headers, or batch boundaries in SQLite or logs.
6. **Roll out behind a transport flag.** Ship individual reads as the fallback and enable batching by default only after live-account benchmarks show a material wall-clock improvement without a higher 429 rate. Keep batch size and quota pace independently configurable so lowering one does not silently lower the other. If batching fails its benchmark because the run is quota-bound rather than connection-bound, retain gzip plus partial responses and do not claim batching solves the quota ceiling.

Acceptance gates:

- A 200-message cache completes with exactly 200 unique success/failure outcomes and no artificial stop near 100–110; a 500+ message test proves list and batch pagination/chunking.
- With a 25-part batch, a successful 200-message hydration uses at most eight outer batch requests while consuming the same 4,000 Gmail quota units as 200 individual `messages.get` calls.
- Mixed batch responses (2xx, 404, 429, 5xx), out-of-order parts, missing `Content-ID`, malformed boundaries, timeouts, and cancellation have deterministic tests; only retryable failed parts are retried.
- Gzip request headers and decompression are verified for both individual and batch transports, and partial-response tests demonstrate that no required body/header data is lost.
- On a representative live 200-message snapshot, default-on batching must reduce wall-clock time by at least 25% and outer HTTP requests by at least 80%, with no increase in quota units, uncaught quota errors, missing cache rows, or sensitive diagnostic output. Otherwise it remains opt-in.
