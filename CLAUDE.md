...
- **Redraw discipline**: both the list and the read view clear the viewport
  ...
  Anything the user still needs to see across a redraw (an error, "no
  earlier view", an edge-of-list notice) is carried as a notice and printed
  by the next render rather than left in scrollback to be erased. Reply and
  compose flows are the deliberate exception: they print prompts, drafts,
  and send confirmations below the message and wait for a keypress before
...
- Gmail reads use individual `messages.get` requests in a bounded worker pool: `gmail` uses configured `concurrency.gmailReads` (default 8), and `gmail cache` uses 8. Because the shared limiter admits a burst rather than spacing reads evenly, this pool size — not the pace — is what a short run's wall-clock time actually depends on. A slow message occupies only one worker; completed messages advance progress immediately. AI retains its separate concurrency budget. Multipart read batching is removed from these execution paths. Trash actions reuse the labels fetched for the scan when building the durable action plan; they do not issue a second `messages.get` per candidate after classification.
...
- **Superseded (the limiter now bursts).** An earlier design spaced reads evenly at the 275/minute target, so one hundred individual reads spent about 22 seconds in admission alone even when Gmail's minute budget was almost entirely unused. That converted a per-minute *bucket* into a per-request metronome, which is not what the quota is. The limiter is now a token bucket whose depth is the whole rolling minute budget: a run spends its allowance as fast as its worker pool can, and waits only once the rolling 60-second window (still the hard cap, still 275 units) is genuinely full. `gmail --limit 100` is therefore bounded by concurrency and Gmail's own latency — a few seconds — not by 100 × 218 ms of self-imposed delay. Sustained runs are unchanged: the same 275 units/minute ceiling applies, so bursting changes *when* the allowance is spent, never how much.
